/**
 * ClapBack AI — Claude Sonnet 4.5 only.
 * Always reads the screenshot. Always contextual. Zero templates.
 */
...
const STORAGE_KEY = 'clapback_ai_key';

export function getAiConfig(): AiConfig | null {
  try {
    const key = localStorage.getItem(STORAGE_KEY);
    if (key?.trim()) return { apiKey: key.trim() };
    return null;
  } catch {
    return null;
  }
...
export function saveAiConfig(apiKey: string): void {
...
export async function generateWithAI(params: AiGenerateParams): Promise<string> {
  ...
  // Clean up any accidental quotes or prefixes the model might add
